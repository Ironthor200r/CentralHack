...
class RiskPredictor:
    ...
    def _calculate_exposure_risk(self, resource: CloudResource) -> float:
        if resource.type == ResourceType.SERVERLESS:
            ...
            has_secrets = any(k.lower() in v.lower() for k, v in env_vars.items() 
...
def badalIChooseYou():
    ...
    try:
        ...
        print("\nDependency graph has been saved as 'dependency_graph.png'")
    except Exception as e:
        ...
        for resource in resources:
            try:
                ...
                if vulns:
                    ...
                    for vuln in vulns:
                        print(f"  - {vuln['id']} ({vuln['severity']}) [CVSS: {vuln['cvss_score']}]")
    ...
    if bottlenecks:
        for bn in bottlenecks:
            ...
            if bn['type'] in ['Single Point of Failure', 'AWS Service Role']:
                print(f"- Resource: {bn['resource']}")
                print(f"- Dependents: {len(bn['dependents'])} resources")
            elif bn['type'] == 'Circular Dependency':
                print(f"- Dependency Chain: {' → '.join(bn['chain'])}")
    else:
    ...
    for bn in bottlenecks:
        print(f"\n🔧 For {bn['type']}:")
        print('\n'.join(remediator.generate_remediation(bn)))
    ...
    for resource_id, info in dependencies.items():
        ...
        if deps:
            for dep in deps:
                print(f"- Depends on: {dep['name']} ({dep['type']})")
        else:
